...
- **Shims** (`use/self-improve/shared-sbin/`): distro-agnostic. The 13 lazy shims (`python`, `python3`, `pip`, `pip3`, `pipx`, `node`, `npm`, `go`, `gh`, `mise`, `pg_dump`, `claude`, `psql`) plus the shared `add-mise` installer and three sidecar scripts (`on-host-podman`, `on-host-sensible-do`, `on-host-commit`) and `lib/shim-common.sh`. One source of truth, shared by every variant.
