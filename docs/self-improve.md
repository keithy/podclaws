...
Per-version and venv packages can coexist: `[tools].python.postinstall` installs into the core interpreter, while venv packages live in the venv itself. The venv takes precedence on `PATH`, with the core interpreter as a fallback.