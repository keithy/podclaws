...
- [ ] **Create k8s/Makefile and test Kubernetes support** — `k8s/Makefile` does not exist yet. Scope: define what "Kubernetes support" means for goclaw (deployment manifests? operator? CI runner?). Create the Makefile, add basic targets (build/deploy/test), and verify against a test cluster or kind/minikube.
  ...
  **What podclaws needs to preserve** (compose translators don't handle these):
  ...
  4. **Mise shared volumes (`mise-musl`, `mise-glibc`, `mise-cache`)** — `hostPath` to the ZFS-backed `/srv/auto_*` paths. Single-node only.
...
- [ ] **Agent: detect shim stubs and trigger the right `add-*` install** — The lazy shims in `use/self-improve/sbin/{python,node,npm,go,gh,mise}` now spoof `--version`/`-V`/`-v` with a self-describing stub string (e.g. `python3 3.12.13 (shim stub — install via add-python)`). The agent should:
  ...
  **Foundation in place** (as of 2026-06-16):
  - `use/self-improve/sbin/lib/shim-common.sh` — `shim_main` helper.
  - 10 shims use it: `python`, `python3`, `pip`, `pip3`, `pipx`, `node`, `npm`, `go`, `gh`, `mise`. `claude` and `psql` have custom flows and are out of scope.
