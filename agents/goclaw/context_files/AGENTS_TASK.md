...
## Host Management via Sensible

You are running inside a rootless Podman container. You manage the host and the running GoClaw service via **sensible**, a secure queue-based execution layer using `execlineb` scripts.
- To execute commands on the host: `sensible do "script content"`
- Always use `execline` syntax for `sensible do` (e.g., no `$VAR` expansion, no `&&` or `||` shell operators inside the string, use `foreground { cmd } cmd2`).
- Example: `sensible do "foreground { make build } make deploy"`
- To check task status: `sensible status <file_id>`
- To list pending tasks: `sensible list`
- Sensible handles the security boundary, allowing you to trigger host actions securely from within your container.
